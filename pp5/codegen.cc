...
Location *CodeGenerator::GenTempVar(int *tmp_offset) {
  static int nextTempNum;
  char temp[10];
  sprintf(temp, "_tmp%d", nextTempNum++);
  Location *result = new Location(fpRelative, *tmp_offset, temp);
  *tmp_offset -= 4;
  return result;
}

 
Location *CodeGenerator::GenLoadConstant(int value, int *tmp_offset) {
  Location *result = GenTempVar(tmp_offset);
  code.push_back(new LoadConstant(result, value));
  return result;
}

Location *CodeGenerator::GenLoadConstant(const char *s, int *tmp_offset) {
  Location *result = GenTempVar(tmp_offset);
  code.push_back(new LoadStringConstant(result, s));
  return result;
} 

Location *CodeGenerator::GenLoadLabel(const char *label, int *tmp_offset) {
  Location *result = GenTempVar(tmp_offset);
...
Location *CodeGenerator::GenLoad(Location *ref, int offset, int *tmp_offset) {
  Location *result = GenTempVar(tmp_offset);
  code.push_back(new Load(result, ref, offset));
  return result;
}

Location *CodeGenerator::GenBinaryOp(const char *opName, Location *op1, Location *op2, int *tmp_offset) {
  Location *result = GenTempVar(tmp_offset);
...
Location *CodeGenerator::GenLCall(const char *label, bool fnHasReturnValue, int *tmp_offset) {
  Location *result = fnHasReturnValue ? GenTempVar(tmp_offset) : NULL;
  code.push_back(new LCall(label, result));
  return result;
}

Location *CodeGenerator::GenACall(Location *fnAddr, bool fnHasReturnValue, int *tmp_offset) {
  Location *result = fnHasReturnValue ? GenTempVar(tmp_offset) : NULL;
...
Location *CodeGenerator::GenBuiltInCall(BuiltIn bn, Location *arg1, Location *arg2, int *tmp_offset) {
  Assert(bn >= 0 && bn < NumBuiltIns);
  struct _builtin *b = &builtins[bn];
  Location *result = NULL;

  if (b->hasReturn) result = GenTempVar(tmp_offset);
